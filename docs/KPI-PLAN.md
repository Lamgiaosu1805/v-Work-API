...
kpi_tier_config ✅      # 5 bậc — Giám đốc TTKD cấu hình trọng số mỗi năm theo từng metric
  ttkd_id, metric_code, year
  tiers: [{ level 1–5, weight: 0–1 }]
    weight = trọng số so với KPI tổng TTKD tháng đó
    target Sale = TTKD monthly target × weight
    VD: TTKD-HN target 10 tỷ, bậc 3 weight=0.15 → Sale bậc 3 target = 1.5 tỷ
    (cùng bậc 3 nhưng TTKD-HCM target 20 tỷ → target = 3 tỷ — scale theo quy mô)
  configured_by

kpi_tier_assignment ✅  # Gán bậc tổng cho từng Sale (lịch sử đầy đủ)
  sale_id, ttkd_id
  tier_level (1–5)      # 1 bậc duy nhất, KHÔNG phân theo metric
...
**ADR-11 — `kpi_tier_assignment` chỉ có 1 bậc tổng, KHÔNG phân theo metric.**
Sale được gán 1 `tier_level` duy nhất trong 1 TTKD — không có "bậc 3 cho REVENUE, bậc 4 cho CIF". Đơn giản hóa quản lý và phù hợp thực tế vận hành. Bỏ `metric_code` khỏi `kpi_tier_assignment`.

**ADR-12 — `kpi_tier_config.weight` là trọng số (0–1) so với KPI tổng TTKD, không phải giá trị tuyệt đối.**
Lý do: mỗi TTKD có quy mô khác nhau — dùng giá trị tuyệt đối thì config không tái sử dụng được. Dùng trọng số: `target Sale = TTKD monthly target × weight`. Cùng bậc 3 (weight=0.15): TTKD-HN 10 tỷ → 1.5 tỷ; TTKD-HCM 20 tỷ → 3 tỷ. Config 1 lần dùng được cho mọi quy mô. `metric_code` vẫn giữ trong tier_config vì mỗi metric có trọng số riêng trong cùng 1 bậc.
...
- 2026-06-30 — [Phase 1 — DB] — Điều chỉnh 2 model tier sau review: (1) `kpi_tier_assignment` bỏ `metric_code` — 1 Sale chỉ có 1 bậc tổng (ADR-11). (2) `kpi_tier_config` đổi `threshold` → `weight` (0–1), là trọng số so với KPI tổng TTKD (ADR-12). — `src/models/KpiTierAssignmentModel.js`, `src/models/KpiTierConfigModel.js`.
- 2026-07-01 — [Phase 2/F02] — `KpiYearPlanController` + route đầy đủ. 7 endpoints: list/getById/create/update/activate/adjustMonthly/remove. Phân rã equal (year_target/12). `ttkd_id` truyền từ body, validate account có thuộc TTKD đó (hỗ trợ 1 người nhiều TTKD). Admin bypass. Chặn chỉnh tháng đã qua. Transaction khi activate. Mount `/kpi/year-plans`. — `src/controllers/KpiYearPlanController.js`, `src/routes/kpiYearPlan.js`.
- 2026-07-01 — [Phase 5] — `KpiTierConfigController` + route đầy đủ. 5 endpoints: list/getById/create/update/remove. Không có versioning — unique index `{ttkd_id, metric_code, year}`. Validate tiers: no dup level, weight 0–1, số bậc tùy ý. Ownership check từ document (không trust body). Mount `/kpi/tier-configs`. — `src/controllers/KpiTierConfigController.js`, `src/routes/kpiTierConfig.js`.
- 2026-07-01 — [Refactor] — Tách `getAccountTtkdIds()` (account → user_info → user_department_position → department type=branch) thành helper dùng chung, xóa bản copy-paste khỏi 2 controller. — `src/helpers/kpiHelper.js`, sửa `KpiYearPlanController.js` + `KpiTierConfigController.js`.
- 2026-07-01 — [Phase 5] — `KpiTierAssignmentController` + route đầy đủ. 4 endpoints: list/getById/assign/remove. Không có update riêng — gọi lại `assign` để đổi bậc, transaction đóng bản ghi active cũ (`effective_to=now`) + tạo bản ghi mới (`effective_to=null`), giữ lịch sử. Nhiều Sale có thể cùng 1 tier_level (weight áp dụng độc lập per Sale, không chia đều — theo yêu cầu). Không validate tier_level khớp `kpi_tier_config`. Mount `/kpi/tier-assignments`. — `src/controllers/KpiTierAssignmentController.js`, `src/routes/kpiTierAssignment.js`.
- 2026-07-01 — [Docs] — Swagger chi tiết cho `kpi_year_plan`, `kpi_tier_config`, `kpi_tier_assignment` (schema + DTO + response + mô tả nghiệp vụ từng field). — `src/docs/{kpiYearPlan,kpiTierConfig,kpiTierAssignment}.yaml`.
- 2026-07-01 — [Phase 2/3] — `kpi_period_target`: CRUD (read) + engine sync + cron + nút đồng bộ thủ công. Engine `syncInvestmentRevenue({year,month,ttkdId})` đọc `investments` (invested_at trong tháng, có commission.sale_id) → gom theo Sale + TTKD, period_type **day + month**, ghi `actual`/`achievement_pct`/`source_breakdown` vào `kpi_period_target` (KHÔNG đụng base_target, bỏ qua record `is_closed`). Controller: list (scope theo TTKD/Sale của account), getById, close (chốt kỳ — quyền `kpi.monthend.close`), sync (POST /sync — quyền `kpi.monthend.close`, non-admin bắt buộc ttkd_id của mình). Cron `kpiSyncJob` chạy 23:30 hàng ngày sync tháng hiện tại. Helpers `kpiPeriod.js` (period_key theo giờ VN) + `kpiSync.js`, `getSaleInfoIdsInTtkds` thêm vào `kpiHelper.js`. Mount `/kpi/period-targets`. **Phạm vi tạm:** chỉ metric `investment_revenue`; source_breakdown MKT/CBB/BLĐ map provisional (Q3 chưa chốt); doanh số attribute theo commission.sale_id (đầu tư của agent/marketing không sale_id chưa tính vào TTKD). — `src/helpers/{kpiPeriod,kpiSync,kpiHelper}.js`, `src/controllers/KpiPeriodTargetController.js`, `src/routes/kpiPeriodTarget.js`, `src/jobs/kpiSyncJob.js`, đăng ký `src/jobs/index.js` + `src/routes/index.js`.
- 2026-07-01 — [Branch] — Tách công việc `kpi_period_target` sang branch riêng `feat/kpi-period-target` (từ `develop`, sau khi `feat/kpi-assignment` đã merge qua PR #63). `docs/` bị xóa khỏi lịch sử git chủ trương của team (commit "Delete docs directory") — `docs/KPI-PLAN.md` tiếp tục dùng làm tài liệu làm việc local, không commit vào git.
- 2026-07-01 — [Docs] — Swagger chi tiết cho `kpi_period_target` (schema, sync DTO, summary response, mô tả rõ giới hạn tạm thời/field nào do engine ghi vs field nào chờ phân rã assignment). — `src/docs/kpiPeriodTarget.yaml`.
- 2026-07-01 — [Phase 4] — `kpi_adjustment` (clawback) + engine trừ actual đầy đủ. Logic đơn giản hóa từ quy tắc 3 tầng tuần/tháng/năm (BRD) xuống 2 tầng ngày/tháng/năm — do `kpi_period_target` hiện chỉ populate day+month: check `is_closed` của record tháng gốc → chưa chốt trừ day+month, đã chốt trừ year (upsert nếu chưa có record). 1 lần clawback tạo nhiều `kpi_adjustment` row (1 row/period_type), mỗi row trừ đồng thời cả scope=sale lẫn scope=ttkd. `amount` mặc định = investment.amount, override được. `metric_code` hardcode `investment_revenue` (nhất quán phạm vi engine sync). Chặn clawback trùng cho cùng investment_id (409). `remove` hoàn tác — cộng lại actual đã trừ. Quyền dùng tạm `kpi.monthend.close` (chưa có permission riêng). Transaction cho cả create lẫn remove (ghi nhiều collection). Mount `/kpi/adjustments`. — `src/helpers/kpiClawback.js`, `getSaleTtkdId` thêm vào `kpiHelper.js`, `src/controllers/KpiAdjustmentController.js`, `src/routes/kpiAdjustment.js`.
- 2026-07-01 — [Docs] — Swagger chi tiết cho `kpi_adjustment` (schema, create/response DTO, mô tả rõ quy tắc xác định kỳ bị trừ + hành vi transaction create/remove). — `src/docs/kpiAdjustment.yaml`.
