...
const AgentController = {
    ...
    try {
      ...
      if (!app_code || !agent_code || !external_id || !full_name || !phone_number || !branch_name) {
        return res.status(400).json({
          message:
            "Thiếu app_code, agent_code, external_id, full_name, phone_number hoặc branch_name"
        });
      }
      ...
      const agent = await AgentModel.findOneAndUpdate(
        { app_id: app._id, agent_code },
        {
          ...
          agent_code,
          external_id,
          agent_type: agent_type || "INDIVIDUAL",
          full_name,
          phone_number,
          email: email ?? null,
  ...
  getAll: async (req, res) => {
    try {
      const { app_code, page = 1, limit = 20, search, is_active } = req.query;
      ...
      const [agents, total] = await Promise.all([
        AgentModel.find(filter).sort({ createdAt: -1 }).skip(skip).limit(Number(limit)),
