...
const formatTerm = (value, type) => (type === "month" ? `${value} tháng` : `${value} ngày`);
...
// Trả về { type: 'all'|'sale', saleIds: null|ObjectId[] }, suy trực tiếp từ Data Scope Policy
// đã gán cho permission "customer.ai_insight" (CUSTOMER_ALL_COMPANY hoặc CUSTOMER_SELF_ASSIGNED)
// — không đọc role/module_access/dept_scope.

const resolveScope = (ability) => {
  const scopeFilter = toMongoQuery(ability, "customer.ai_insight", "Customer");
  if (Object.keys(scopeFilter).length === 0) return { type: "all", saleIds: null };

  const saleId = scopeFilter.referred_by?.$eq ?? null;
  return { type: "sale", saleIds: saleId ? [saleId] : [] };
};
...
exports.customerSummary = async (req, res) => {
  ...
  try {
    ...
    const scopeFilter = toMongoQuery(req.permissionAbility, "customer.ai_insight", "Customer");
    const [customer, investments] = await Promise.all([
      CustomerModel.findOne({ _id: customerId, isDeleted: false, ...scopeFilter })
        .select(
          "identity.full_name identity.date_of_birth status source_type createdAt phone_number"
        )
        .lean(),
      InvestmentModel.find({ customer_id: customerId, isDeleted: false })
        .select(
          "product_name amount term_type term_value interest_rate invested_at maturity_at status"
        )
        .sort({ invested_at: -1 })
        .limit(10)
        .lean()
...
exports.getChurnRisks = async (req, res) => {
  try {
    ...
    const cacheKey = saleId
      ? `${ENV_PREFIX}:ai:churn_risks:sale:${saleId}`
      : `${ENV_PREFIX}:ai:churn_risks:all`;
    const cached = await redis.get(cacheKey);
...
const _computeChurnRisks = async (saleId = null) => {
  ...
  const atRiskCustomers = await CustomerModel.aggregate([
    ...
    {
      $match: {
        $or: [{ lastInvestment: { $size: 0 } }, { "lastInvestment.0.invested_at": { $lt: cutoff } }]
      }
    },
    {
      $lookup: {
...
const buildSystemPrompt = (scopeType) => {
  ...
  return `${CHAT_SYSTEM_PROMPT_BASE}\nQUAN TRỌNG: Bạn chỉ được phép tra cứu và trả lời về khách hàng do nhân viên này trực tiếp phụ trách. Tuyệt đối không tiết lộ thông tin ngoài phạm vi quyền hạn.`;
};
...
const executeTool = async (toolName, toolInput, scope) => {
  ...
  if (toolName === "search_customer") {
    ...
    if (!customers.length)
      return "Không tìm thấy khách hàng phù hợp trong phạm vi quyền hạn của bạn.";
...
exports.chat = async (req, res) => {
  ...
  try {
    const scope = resolveScope(req.permissionAbility);
