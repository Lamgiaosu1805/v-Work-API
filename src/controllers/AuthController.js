...
const AuthController = {
  ...
  login: async (req, res) => {
    try {
      ...
      const refreshToken = jwt.sign({ id: account._id }, JWT_REFRESH_SECRET, { expiresIn: "10d" });
  ...
  refreshToken: async (req, res) => {
    try {
      ...
      const newRefreshToken = jwt.sign({ id: decoded.id }, JWT_REFRESH_SECRET, { expiresIn: "3d" });
  ...
  logout: async (req, res) => {
    try {
      ...
      if (!account) return res.status(404).json({ message: "Không tìm thấy tài khoản hoặc token" });

      // Revoke token
      const tokenEntry = account.refreshTokens.find((t) => t.token === refreshToken);
