...
const HolidayController = {
    getHolidays: async (req, res) => {
        try {
            ...
            const allBranchIds = [...new Set(groups.flatMap(g => g.branches.map(b => b.toString())))];
            const branchDocs = allBranchIds.length
                ? await BranchModel.find({ _id: { $in: allBranchIds } }, "branch_name branch_code")
                : [];
    ...
    createHoliday: async (req, res) => {
        try {
            const { date, name, duration_days = 1, scope_type, branches, pay_policy } = req.body;
            if (!date || !name) return res.status(400).json({ message: "date và name là bắt buộc" });

            const startMoment = moment.tz(date, TZ).startOf("day");
            if (!startMoment.isValid()) return res.status(400).json({ message: "date không hợp lệ" });
            if (duration_days <= 0) return res.status(400).json({ message: "duration_days phải lớn hơn 0" });

            if (scope_type === "branch" && (!branches || !branches.length))
                return res.status(400).json({ message: "Phạm vi theo chi nhánh yêu cầu chọn ít nhất 1 chi nhánh" });
            ...
            if (duration_days <= 1) {
                const existing = await HolidayModel.findOne({ date: startMoment.toDate(), isDeleted: false });
                if (existing) return res.status(409).json({ message: "Ngày lễ này đã tồn tại" });
                ...
                const populated = await holiday.populate("branches", "branch_name branch_code");
                return res.status(201).json({ message: "Tạo ngày lễ thành công", data: populated });
            }
            ...
            const existingDates = await HolidayModel.find({
                date: { $gte: startMoment.toDate(), $lte: dates[dates.length - 1].toDate() },
                isDeleted: false,
            }).select("date");
            const existingSet = new Set(existingDates.map(h => moment.tz(h.date, TZ).format("YYYY-MM-DD")));

            const toCreate = dates.filter(d => !existingSet.has(d.format("YYYY-MM-DD")));
            const skipped = dates.filter(d => existingSet.has(d.format("YYYY-MM-DD"))).map(d => d.format("DD/MM/YYYY"));
    ...
    updateHoliday: async (req, res) => {
        try {
            const { id } = req.params;
            const { date, name, duration_days, scope_type, branches, pay_policy } = req.body;
