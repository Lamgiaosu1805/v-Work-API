...
} = require("../helpers/commissionCalculator");
...
function buildPeriodRange(period) {
  ...
  if (period === "week") {
    ...
    return {
      ...
      buckets,
      groupId: {
        year: { $isoWeekYear: "$invested_at" },
        week: { $isoWeek: "$invested_at" },
      },
  ...
  return {
    ...
    buckets,
    groupId: {
      year: { $year: "$invested_at" },
      month: { $month: "$invested_at" },
    },
...
const InvestmentController = {
  ...
  upsert: async (req, res) => {
    ...
    try {
      ...
      if (!["week", "month"].includes(term_type)) {
        ...
        return res
          .status(400)
          .json({ message: "term_type phải là 'week' hoặc 'month'" });
      }
      ...
      if (existing) {
        ...
        if (
          ["early_terminated", "cancelled"].includes(status) &&
          existing.commission?.status === "pending"
        ) {
      ...
      if (term_type === "month") {
        if (customer.referred_by) {
          const sale = await UserInfoModel.findById(
    ...
    try {
      ...
      const [investments, total] = await Promise.all([
        InvestmentModel.find(filter)
          ...
          .select(
            "product_name amount term_type term_value interest_rate invested_at maturity_at status commission createdAt",
          )
          .sort({ invested_at: -1 })
          .skip(skip)
          .limit(Number(limit)),
      ...
      const customerCommissions = await CustomerModel.find({
        ...
        "phone_number external_id identity.full_name cif_commission ekyc_commission",
      );

      const cifCount = customerCommissions.filter(
        (c) =>
          c.cif_commission?.sale_id?.toString() === sale._id.toString() &&
          c.cif_commission?.granted_at >= startOfMonth,
      ).length;
      const ekycCount = customerCommissions.filter(
        (c) =>
          c.ekyc_commission?.sale_id?.toString() === sale._id.toString() &&
      ...
      return res.status(200).json({
        ...
        customer_commission_summary: {
          ...
          total_amount:
            cifCount * CIF_COMMISSION_AMOUNT +
            ekycCount * EKYC_COMMISSION_AMOUNT,
        },
    ...
    try {
      ...
      if (sale_id) {
        ...
        const customerCommissions = await CustomerModel.find({
          ...
          "phone_number identity.full_name cif_commission ekyc_commission",
        );

        const cifCount = customerCommissions.filter(
          (c) =>
            c.cif_commission?.sale_id?.toString() === String(sale._id) &&
            c.cif_commission?.granted_at >= startOfMonth,
        ).length;
        const ekycCount = customerCommissions.filter(
          (c) =>
            c.ekyc_commission?.sale_id?.toString() === String(sale._id) &&
            c.ekyc_commission?.granted_at >= startOfMonth,
        ).length;
      ...
      const [invAgg, cifAgg, ekycAgg] = await Promise.all([
        InvestmentModel.aggregate([
          ...
          {
            $group: {
              _id: "$commission.sale_id",
              inv_count: { $sum: 1 },
              inv_net: { $sum: "$commission.net_amount" },
              inv_gross: { $sum: "$commission.gross_amount" },
            },
          },
        ]),
        CustomerModel.aggregate([
          {
            $match: {
              "cif_commission.sale_id": { $exists: true, $ne: null },
              "cif_commission.granted_at": {
                $gte: startOfMonth,
                $lte: endOfMonth,
              },
        ...
        CustomerModel.aggregate([
          {
            $match: {
              "ekyc_commission.sale_id": { $exists: true, $ne: null },
              "ekyc_commission.granted_at": {
                $gte: startOfMonth,
                $lte: endOfMonth,
              },
      ...
      const saleInfos = await UserInfoModel.find({ _id: { $in: allSaleIds } })
        .select("full_name ma_nv phone_number")
        .lean();
      const saleMap = {};
      saleInfos.forEach((s) => {
        saleMap[String(s._id)] = s;
      });
      ...
      const invMap = {},
        ...
        ekycMap = {};
      invAgg.forEach((i) => {
        invMap[String(i._id)] = i;
      });
      cifAgg.forEach((i) => {
        cifMap[String(i._id)] = i;
      });
      ekycAgg.forEach((i) => {
        ekycMap[String(i._id)] = i;
      });

      const rows = allSaleIds
        .map((sid) => {
  ...
  getAgentCommission: async (req, res) => {
    try {
      ...
      if (!app_code || !agent_code) {
        return res
          .status(400)
          .json({ message: "Thiếu app_code hoặc agent_code" });
      }
    ...
    try {
      ...
      const [investments, total] = await Promise.all([
        InvestmentModel.find(filter)
          ...
          .select(
            "product_name amount term_type term_value interest_rate invested_at maturity_at status commission",
          )
          .sort({ invested_at: -1 })
          .skip(skip)
          .limit(Number(limit)),
  ...
  bulkSync: async (req, res) => {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "Thiếu app_code hoặc investments" });
      }

      if (investments.length > 500) {
        return res
          .status(400)
          .json({ message: "Tối đa 500 khoản đầu tư mỗi lần" });
      }
  ...
  getSalesChart: async (req, res) => {
    try {
      ...
      const isMgr =
        role === "manager" && req.account.module_access.includes("crm");
      ...
      if (isAdmin) {
        ...
        if (branch_id) {
          const branchSaleIds = await UserInfoModel.distinct("_id", {
            branch_id: new mongoose.Types.ObjectId(branch_id),
            isDeleted: false,
          });
          const branchCustomerIds = await CustomerModel.distinct("_id", {
            referred_by: { $in: branchSaleIds },
            isDeleted: false,
          });
        ...
        const deptSaleIds = await UserDepartmentPositionModel.distinct("user", {
          department: { $in: myDeptIds },
          isDeleted: false,
        });
        const deptCustomerIds = await CustomerModel.distinct("_id", {
          referred_by: { $in: deptSaleIds },
          isDeleted: false,
        });
        ...
        if (!myInfo)
          return res
            .status(404)
            .json({ message: "Không tìm thấy thông tin nhân viên" });
        const myCustomerIds = await CustomerModel.distinct("_id", {
          referred_by: myInfo._id,
          isDeleted: false,
        });
      ...
      const seriesMap = {};
      for (const r of timeRaw)
        seriesMap[formatKey(r._id)] = { amount: r.amount, count: r.count };
      ...
      if (mode === "all") {
        const branchBreakRaw = await InvestmentModel.aggregate([
          { $match: { invested_at: { $gte: from }, isDeleted: false } },
          {
            $lookup: {
              ...
              foreignField: "_id",
              as: "_c",
              pipeline: [{ $project: { referred_by: 1 } }],
            },
          },
          {
          ...
          },
          {
            $lookup: {
              ...
              foreignField: "_id",
              as: "_s",
              pipeline: [{ $project: { branch_id: 1 } }],
            },
          },
          {
            $addFields: {
              branch_id: {
                $ifNull: [{ $arrayElemAt: ["$_s.branch_id", 0] }, null],
              },
            },
          },
          {
        ...
        breakdown = branchBreakRaw.map((r) => ({
          id: r._id,
          name: r._id
            ? (branchMap[String(r._id)] ?? "Chi nhánh khác")
            : "Chưa phân chi nhánh",
          amount: r.amount,
          count: r.count,
        }));
        ...
        const infoDocs = await UserInfoModel.find({ _id: { $in: infoIds } })
          .select("_id full_name ma_nv")
          .lean();
        const infoMap = Object.fromEntries(
          infoDocs.map((u) => [String(u._id), u]),
        );
        breakdown = memberRaw.map((r) => {
          const u = infoMap[String(r._id)];
          return {
            id: r._id,
            name: u?.full_name ?? u?.ma_nv ?? "Không xác định",
            amount: r.amount,
            count: r.count,
          };
        });
      ...
      return res.status(200).json({
        ...
        breakdown,
        summary: {
          total_amount: summaryRaw[0]?.amount ?? 0,
          total_count: summaryRaw[0]?.count ?? 0,
        },
      });
    } catch (error) {
    ...
    try {
      ...
      if (err.response) {
        return res.status(err.response.status).json({
          message: "Lỗi từ hệ thống Tikluy",
...
InvestmentController.getExpiring = async (req, res) => {
  try {
    ...
    const investments = await InvestmentModel.find(match)
      .populate({
        path: "customer_id",
        select: "identity.full_name phone_number",
      })
    ...
    if (
      ...
      const sales = await UserInfoModel.find({ _id: { $in: saleIds } })
        .select("full_name")
        .lean();
      sales.forEach((s) => {
        saleMap[String(s._id)] = s.full_name;
      });
    }
    ...
    res.json({
      ...
      investments: investments.map((i) => {
        ...
        return {
          _id: i._id,
          customer_name:
            i.customer_id?.identity?.full_name ??
            i.customer_id?.phone_number ??
            "—",
          ...
          days_left: daysLeft,
          urgency:
            daysLeft <= 7 ? "urgent" : daysLeft <= 14 ? "warning" : "normal",
    ...
    res
      .status(500)
      .json({ message: "Lỗi lấy danh sách đáo hạn", error: err.message });
  }
};
  ...
  try {
    ...
    if (req.account.role === "user") {
      const userInfo = await UserInfoModel.findOne({
        ...
        .select("_id")
        .lean();
      const depts = await UserDepartmentPositionModel.find({
        user: userInfo?._id,
      })
        .select("department")
        .lean();
      const deptIds = depts.map((d) => d.department);
      const colleagues = await UserDepartmentPositionModel.find({
        department: { $in: deptIds },
      })
        .select("user")
        .lean();
      const colleagueIds = colleagues.map((c) => c.user);
      const scopedCustomerIds = await CustomerModel.distinct("_id", {
        referred_by: { $in: colleagueIds },
        isDeleted: false,
      });
    ...
    const rows = await InvestmentModel.aggregate([
      ...
      },
      {
        $project: {
          ...
          count: 1,
          sale_name: {
            $ifNull: [{ $arrayElemAt: ["$info.full_name", 0] }, "Không rõ"],
          },
        },
      },
    ]);
    ...
    res
      .status(500)
      .json({ message: "Lỗi lấy leaderboard", error: err.message });
  }
};
...
InvestmentController.getConversion = async (req, res) => {
  try {
    ...
    if (!isManager) {
      ...
      const [kycDone, investedIds] = await Promise.all([
        CustomerModel.countDocuments({
          _id: { $in: myCustomerIds },
          status: { $in: ["kyc_verified", "active"] },
          isDeleted: false,
        }),
    ...
    const rows = await CustomerModel.aggregate([
      ...
      },
      {
        $group: {
          ...
          kyc_done: {
            $sum: {
              $cond: [{ $in: ["$status", ["kyc_verified", "active"]] }, 1, 0],
            },
          },
    ...
    res
      .status(500)
      .json({ message: "Lỗi lấy tỷ lệ chuyển đổi", error: err.message });
  }
};
