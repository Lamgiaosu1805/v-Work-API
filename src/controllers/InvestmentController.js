...
function buildPeriodRange(period) {
  ...
  if (period === "day") {
    ...
    return {
      ...
      buckets,
      groupId: {
        $dateToString: { format: "%Y-%m-%d", date: "$invested_at", timezone: "Asia/Ho_Chi_Minh" }
      },
      formatKey: (id) => id
    };
  }
...
const InvestmentController = {
  ...
  // Scope theo Data Scope Policy của "investment.view" — INVESTMENT_ALL_COMPANY (thấy tất cả)
  // hoặc INVESTMENT_SELF_ASSIGNED (chỉ khách hàng do chính mình giới thiệu)
  list: async (req, res) => {
    try {
      const { page = 1, limit = 20, status, date_from, date_to, q } = req.query;

      const filter = toMongoQuery(req.permissionAbility, "investment.view", "Investment");
      ...
      // Tìm kiếm theo số điện thoại hoặc tên khách — merge bằng $and để không đè lên
      // customer_id scope filter đã có sẵn (vd INVESTMENT_SELF_ASSIGNED)
      if (q) {
        ...
        const searchCondition = { customer_id: { $in: matchedCustomers.map((c) => c._id) } };
        if (filter.customer_id) {
          const scopeCondition = { customer_id: filter.customer_id };
          delete filter.customer_id;
          filter.$and = [...(filter.$and || []), scopeCondition, searchCondition];
        } else {
...
InvestmentController.getExpiring = async (req, res) => {
  try {
    ...
    const match = {
      ...
      ...toMongoQuery(req.permissionAbility, "investment.view", "Investment")
    };
    ...
    // Lấy thêm tên sale để hiển thị (không nhạy cảm, hiển thị cho mọi scope)
    const saleIds = [
      ...new Set(
        investments
...
InvestmentController.getLeaderboard = async (req, res) => {
  try {
    ...
    // Scope theo Data Scope Policy của "investment.leaderboard" — INVESTMENT_ALL_COMPANY
    // (manager/admin) hoặc INVESTMENT_OWN_DEPARTMENT (sale thường — xếp hạng cùng phòng ban)
    Object.assign(
      matchInvestment,
      toMongoQuery(req.permissionAbility, "investment.leaderboard", "Investment")
    );
