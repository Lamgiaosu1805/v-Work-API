...
async function getAccountIdsInDept(deptId) {
    const memberships = await UserDepartmentPositionModel.find({ department: deptId, isDeleted: false }).select("user");
    const userIds = memberships.map((m) => m.user);
    const users = await UserInfoModel.find({ _id: { $in: userIds }, isDeleted: false }).select("id_account");
...
cron.schedule("0 8 * * 5", async () => {
    try {
        ...
        for (const dept of departments) {
            ...
            if (!report) {
                report = await WeeklyReportModel.create({ department: dept._id, weekStart, deadline, status: "pending" });
                created++;
            }
        ...
        console.log(`[Weekly Report] Thứ 6 8:00 — Tạo mới ${created} record, nhắc ${pendingDeptIds.length} phòng ban chưa nộp`);
    } catch (err) {
